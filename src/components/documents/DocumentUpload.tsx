...
const DocumentUpload = () => {
  ...
  console.log('DocumentUpload: user:', user, 'profile:', profile);
  ...
  useEffect(() => {
    console.log('DocumentUpload: Initial fetch useEffect triggered');
    if (user) {
      console.log('DocumentUpload: Initial fetch triggered for user:', user.id);
      fetchDocuments();
    ...
    return () => {
      console.log('DocumentUpload component unmounting');
    };
  }, [user, refreshTrigger]);

  useEffect(() => {
    console.log('DocumentUpload: Year change or user change useEffect triggered');
    if (user) {
      console.log('DocumentUpload: Year change or user change detected, fetching timesheet schedules');
  ...
  // Effect to handle refresh trigger changes
  useEffect(() => {
    if (user && refreshTrigger > 0) {
      console.log('Refresh trigger activated, fetching timesheet schedules');
      // Add a delay to ensure any pending updates are completed
      setTimeout(() => {
        fetchTimesheetSchedules();
      }, 300);
    }
  ...
  // Effect to handle window focus/blur events
  useEffect(() => {
    const handleFocus = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uploadTarget) {
        try {
          // Check if this is an update by checking existing submission
      ...
      if (uploadTarget?.type === 'work') {
        ...
        while (!submissionSuccess && submissionAttempts < maxSubmissionAttempts) {
          ...
          try {
            ...
            if (submissionError) {
              console.error(`Attempt ${submissionAttempts}: Error upserting timesheet submission:`, submissionError);
              if (submissionAttempts < maxSubmissionAttempts) {
                // Wait before retrying
                await new Promise(resolve => setTimeout(resolve, 200 * submissionAttempts));
                continue;
              } else {
            ...
            console.log(`Attempt ${submissionAttempts}: Timesheet submission upserted successfully`);
            submissionSuccess = true;
          } catch (submissionErr) {
            console.error(`Attempt ${submissionAttempts}: Exception upserting timesheet submission:`, submissionErr);
            if (submissionAttempts >= maxSubmissionAttempts) {
              throw submissionErr;
            }
            // Wait before retrying
  ...
  const handleTimesheetUploadClick = async (periodData: TimesheetSchedule | undefined, periodNum: number, month: number, year: number) => {
    ...
    if (periodData) {
      ...
      try {
        ...
        if (newPeriodData) {
          setUploadTarget({ periodId: newPeriodData.id, type: 'work' });
          setIsUploadDialogOpen(true);
  ...
  const renderCategoryTab = (categoryKey: keyof typeof documentCategories) => {
    console.log('Rendering category tab:', categoryKey);
