          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center space-x-2">
                ...
                {unreadCount > 0 && <span className="bg-red-500 text-white text-xs px-2 py-1 rounded-full">
                    {unreadCount}
                  </span>}
              </CardTitle>
              <CardDescription>
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 space-y-4 sm:space-y-0">
            <div className="flex space-x-2">
              <Button variant={filter === 'all' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('all')} className="rounded-lg">
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
              ...
              {notifications.some(n => n.read_at) && <Button variant="outline" size="sm" onClick={deleteAllRead} className="rounded-lg text-red-600 hover:text-red-700">
          ...
          {filteredNotifications.length === 0 ? <div className="text-center py-12">
              ...
              <p className="text-gray-500">
                {filter === 'unread' ? 'All caught up! Check back later for updates.' : 'You\'ll receive notifications about your learnership progress here.'}
              </p>
            </div> : <div className="space-y-3">
              {filteredNotifications.map(notification => <div key={notification.id} className={`p-4 rounded-lg transition-all duration-200 hover:shadow-md ${getNotificationBorderColor(notification.type, !!notification.read_at)}`}>
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {!notification.read_at && <div className="h-2 w-2 bg-blue-500 rounded-full"></div>}
                        </div>
                    ...
                    <div className="flex items-center justify-end space-x-1 mt-2 sm:mt-0 sm:ml-4 w-full sm:w-auto">
                      {!notification.read_at && <Button variant="ghost" size="sm" onClick={() => markAsRead(notification.id)} className="h-8 w-8 p-0 rounded-full hover:bg-blue-100" title="Mark as read">
                          <Check className="h-4 w-4 text-blue-600" />
                        </Button>}
                      <Button variant="ghost" size="sm" onClick={() => deleteNotification(notification.id)} className="h-8 w-8 p-0 rounded-full hover:bg-red-100" title="Delete notification">
